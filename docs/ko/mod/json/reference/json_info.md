...
JSON 또는 JSONC 파일을 편집할 때 아래와 같이 올바른 형식을 적용했는지 확인하세요.
...
JSONC 파일에서는 `//` 줄 주석, `/* */` 블록 주석, 끝 쉼표를 사용할 수 있습니다.
...
일부 JSON 및 JSONC 문자열(아이템 이름, 설명 등)은 번역 추출 대상입니다. 정확한 추출 방식은
`scripts/extract_json_strings.ts`에서 처리됩니다. 번역 컨텍스트 없이 문자열을 쓰는 일반적인 방식 외에도,
...
아래처럼 `"//~"` 항목을 추가해 번역자를 위한 설명을 넣을 수도 있습니다. JSONC 파일에서는
번역 가능한 문자열 바로 앞의 `//~` 주석도 사용할 수 있습니다. 항목 순서는 중요하지 않습니다.
