...
문자열 추출과 POT 중복 제거에는 Deno가 필요합니다.

문자열 추출을 위한 스크립트는 리포지토리에서 찾을 수 있습니다:

- `scripts/extract_json_strings.ts` - 기본 문자열 추출 루틴
- `scripts/pot_tools.ts` - 추출기가 생성한 POT 파일의 오류 수정
- `lang/extract_mod_strings.bat` (Linux/MacOS의 경우 `lang/extract_mod_strings.sh`) - 추출과 중복 제거를 자동화합니다.
