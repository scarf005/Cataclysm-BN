...
文字列抽出と POT の重複除去には Deno が必要です。

文字列抽出用スクリプトは、リポジトリにあります。

- `scripts/extract_json_strings.ts` - 主な文字列抽出ルーチン
- `scripts/pot_tools.ts` - 抽出器が生成した POT ファイルのエラーを修正
- `lang/extract_mod_strings.bat` (Linux/MacOS の場合は `lang/extract_mod_strings.sh` ) - 抽出と重複除去を自動化
