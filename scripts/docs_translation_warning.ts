...
const ghEnv = (): Record<string, string> => {
  ...
  return { PATH: Deno.env.get("PATH") ?? "", ...(token === undefined ? {} : { GH_TOKEN: token }) }
}

const gh = (args: string[], input?: string): Promise<string> => {
  let command = new CommandBuilder().command(["gh", ...args]).clearEnv().env(ghEnv())
  if (input !== undefined) command = command.stdinText(input)
  return command.text()
}

const readPrBody = async (pr: number): Promise<string> =>
  (await gh(["pr", "view", String(pr), "--json", "body", "--jq", ".body"])).trimEnd()

const updatePrBody = async (pr: number, body: string): Promise<void> => {
  await gh(["pr", "edit", String(pr), "--body-file", "-"], body)
}
