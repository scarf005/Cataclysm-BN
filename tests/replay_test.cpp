...
auto test_replay_path() -> std::filesystem::path {
    ...
    return std::filesystem::temp_directory_path()
         / ("cataclysm-bn-replay-test-" + std::to_string(process_token) + "-" + std::to_string(id)
            + ".jsonl");
}
...
auto write_replay_inputs(const std::filesystem::path& path, const std::vector<input_event>& events)
    -> void {
    replay::stop();
    replay::configure_recording(path.string());
    replay::start();
    for (const auto& event : events) { replay::record_input_event(event); }
    replay::stop();
}
...

    {
        ...
        for (auto event_index = std::uint64_t{0}; event_index < 2; ++event_index) {
            ...
            for (auto i = std::size_t{0}; i < values.size(); ++i) {
                pool.submit(
                    {.stream = 0x7265706c61795f5fULL, .id = (event_index << 32) | i},
...
TEST_CASE(
    "replay_playback_is_deterministic_with_multithreaded_tasks", "[replay][rng][thread_pool]") {
    const auto path = test_replay_path();
    const auto cleanup = on_out_of_scope([path]() { cleanup_replay_file(path); });
