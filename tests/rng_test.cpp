...
TEST_CASE("deterministic_rng_normal_roll_uses_scope_engine", "[rng][thread_pool]") {
    ...
    const auto expected_second =
        expected_distribution(expected_engine, std::normal_distribution<>::param_type(10.0, 2.0));
...
TEST_CASE("deterministic_rng_task_scope_restores_nested_child_counter", "[rng][thread_pool]") {
...

    {
        ...
        CHECK(
            *first_child == rng_deterministic_child_seed(outer_seed, {.stream = stream, .id = 0}));
        CHECK(
            *second_child == rng_deterministic_child_seed(outer_seed, {.stream = stream, .id = 1}));

        {
            ...
            CHECK(*nested_child == rng_deterministic_child_seed(1234, {.stream = stream, .id = 0}));
        }

        const auto after_nested = rng_next_deterministic_call_seed(stream);
        REQUIRE(after_nested);
        CHECK(
            *after_nested == rng_deterministic_child_seed(outer_seed, {.stream = stream, .id = 2}));
    }
...
TEST_CASE("deterministic_rng_scopes_parallel_tasks", "[rng][thread_pool]") {
    ...
    const auto run_submitted_tasks = []() {
        ...
        for (auto i = std::size_t{0}; i < values.size(); ++i) {
            pool.submit({.stream = 0x746573745f737562ULL, .id = i}, [&values, &done, i]() {
...
TEST_CASE("deterministic_rng_keyed_tasks_ignore_submission_order", "[rng][thread_pool]") {
    const auto run_order = [](const std::vector<std::size_t>& order) {
        ...
        for (const auto i : order) {
            pool.submit({.stream = 0x6f726465725f5f5fULL, .id = i}, [&values, &done, i]() {
                values[i] = rng(1, 1000000);
                done.count_down();
            });
        }
    ...
    CHECK(run_order({0, 1, 2, 3, 4, 5, 6, 7}) == run_order({7, 6, 5, 4, 3, 2, 1, 0}));
}
...
TEST_CASE("deterministic_rng_scopes_submit_returning_without_workers", "[rng][thread_pool]") {
    ...
    const auto first =
        pool.submit_returning(
                {.stream = 0x73796e635f5f5f5fULL, .id = 42},
                []() {
