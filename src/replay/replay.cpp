...
{
    if( type == "keyboard" ) {
        return input_event_t::keyboard;
    }
    if( type == "gamepad" ) {
        return input_event_t::gamepad;
    }
    if( type == "mouse" ) {
        return input_event_t::mouse;
    }
    if( type == "timeout" ) {
        return input_event_t::timeout;
    }
...
auto should_store_event( const input_event &event ) -> bool
{
    return event.type == input_event_t::keyboard || event.type == input_event_t::gamepad ||
           event.type == input_event_t::mouse;
}
...
auto read_event( const std::string &line ) -> input_event
{
    ...
    if( mouse_pos.size() == 2 ) {
        event.mouse_pos = point( mouse_pos[0], mouse_pos[1] );
    }
...
{
    return configured_mode() != mode::none;
}

auto is_recording() -> bool
{
    return configured_mode() == mode::record;
}

auto is_playing() -> bool
{
    return configured_mode() == mode::playback;
}
...
auto start() -> void
{
    ...
    if( current.path.empty() ) {
        throw std::runtime_error( "Replay path must not be empty" );
    }
    if( current.active_mode == mode::record ) {
        current.record_stream.open( current.path, std::ios::out | std::ios::trunc | std::ios::binary );
...
{
    auto &current = state();
    if( current.record_stream.is_open() ) {
        current.record_stream.close();
    }
...
{
    auto &current = state();
    if( current.active_mode != mode::record || !should_store_event( event ) ) {
        return;
    }
...
{
    auto &current = state();
    if( current.active_mode != mode::playback ) {
        return std::nullopt;
    }
