...
auto shelf_life_of( const item &value ) -> std::optional<rot::shelf_life>
{
    ...
    return rot::shelf_life::from_duration( value.is_food() ? value.get_comestible()->spoils :
                                           24_hours );
}
...
{
    if( target ) {
        target->borrowed_rot_state = state;
    }
}
...
{
    if( target ) {
        target->borrowed_rot_state = component_rot_state::none;
    }
}
...
{
    const auto lifetime = shelf_life_of( *this );
    if( !lifetime || has_flag( flag_PROCESSING ) ) {
        return;
    }
...
{
    const auto lifetime = shelf_life_of( *this );
    if( !lifetime || rot_is_suspended() || is_in_preserving_container() ) {
        return 0_turns;
    }
    return calc_rot_interval( time, temp, *lifetime );
}

auto item::calc_rot_interval( time_point time, units::temperature temp,
                              const rot::shelf_life &lifetime ) const -> time_duration
{
    // Sealed food past twice its shelf life needs no further aging; corpses do.
    if( !is_corpse() && lifetime.relative_rot( rot ) > 2.0 ) {
        return 0_turns;
    }
                        ...
                        const absolute_rot_process_options &options ) -> detached_ptr<item>
{
    ...
    if( !lifetime || self->rot_is_suspended() ) {
        return std::move( self );
    }
...
auto clip_by_temperature_flag( units::temperature temperature,
                               temperature_flag flag ) -> units::temperature
{
    switch( flag ) {
        case temperature_flag::TEMP_NORMAL:
