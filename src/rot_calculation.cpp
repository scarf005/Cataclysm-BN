...
std::expected<time_duration, relative_rot_error>
{
    if( !std::isfinite( fraction ) ) {
        return std::unexpected( relative_rot_error::non_finite );
    }
...
auto shelf_life::minimum_freshness( time_duration accumulated,
                                    units::temperature temperature ) const -> time_duration
{
    ...
    return time_duration::from_turns( static_cast<int>( std::min(
                                          turns, to_turns<int64_t>( calendar::INDEFINITELY_LONG_DURATION ) ) ) );
}
...
auto increment( const interval_options &options ) -> time_duration
{
    ...
    const auto added = static_cast<int64_t>( static_cast<double>( adjusted ) /
                       to_turns<int>( 1_hours ) * get_hourly_rotpoints_at_temp( options.temperature ) );
